...
Incorporating these surrogate safety measures allows relating behavioral regimes and maneuver-execution modes to the surrounding traffic context, supporting interpretability and exploratory analysis.
%\cite{wang_review_2021, bahmankhah_interaction_2019, hu_high-resolution_2022, huang_predicting_2024, nabavi_niaki_is_2019}

% infrastructure
Furthermore, cyclist behavior is strongly influenced by infrastructure. Empirical studies have shown substantial variability in riding behavior as a function of contextual factors such as topography, curvature, intersections, and wind conditions. For example \citeauthor{belikhov_exploring_2025} (\citeyear{belikhov_exploring_2025}) shows that a large proportion of variation in speed and power output during unconstrained riding can be explained by infrastructure characteristics and environmental influences, highlighting the adaptive nature of cyclist behavior. Similarly, overtaking maneuvers have been shown to depend on infrastructural and traffic-related factors. Using drone-based trajectory data, \citeauthor{wang_investigating_2022} (\citeyear{wang_investigating_2022}) identify significant relations between lane width, vehicle types, and rider characteristics in overtaking decisions. Beyond observed behavior, perceived safety has also been linked to specific infrastructure design elements such as seperation from motorized traffic and lane width \cite{gossling_subjectively_2022, von_stulpnagel_how_2022}.

While traffic and infrastructure can influence behavior, the identification of local regimes in this work is performed independently of such environmental factors. Contextual variables are instead analyzed post hoc to interpret observed behavioral patterns, providing descriptive insight into how maneuvers manifest under varying traffic and infrastructural conditions without affecting the unsupervised abstraction of micro-level states.
...
Unsupervised clustering requires assessing the structural stability, validity and interpretability of the resulting clusters. 
In contrast to supervised learning, no ground truth labels are available for direct performance evaluation. Consequently, clustering quality is typically examined through internal geometric criteria such as compactness and separation, as well as through external agreement measures across runs, initializations, and perturbation.
...
Given the hierarchical and exploratory nature of the proposed framework, cluster stability and interpretability were prioritized over pure geometric separation. Therefore, model selection does not rely exclusively on maximum Silhouette score but incorporates stability under perturbation as well as quantitative assessment of cluster spread.
