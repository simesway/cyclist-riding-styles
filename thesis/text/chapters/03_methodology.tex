...
Since riding in and around intersections differ in dynamics from riding in straight street segments, the windows were split in two groups depending on the distance to a traffic light (easier to obtain than center of intersection). The intersection- and non-intersection-windows were clustered seperately.
...
To assess clustering stability, the performance was evaluated through the Adjusted Rand Index (ARI) and silhouette score across multiple runs. 

Clustering stability was evaluated using the Adjusted Rand Index (ARI) and silhouette score across multiple runs. Stability was assessed via subsampling, random seed variation, and Gaussian noise scaled to each feature’s standard deviation, with ARI computed relative to a reference clustering. Silhouette scores were calculated for the reference clustering, and aggregated statistics (median, mean, standard deviation, 10th and 90th percentiles) summarized reliability, with single-cluster cases treated as undefined.


\subsection{Grid Search}
A grid search was performed to identify hyperparameters that yield interpretable clusters while maintaining high clustering quality, measured by the silhouette score and balanced sample distribution across clusters, while ensuring a minimum cluster size. For each combination, features were selected, PCA applied, and KMeans clustering performed. Stability metrics, including silhouette and Adjusted Rand Index (ARI) scores, were estimated over several runs on an 80\% stratified subsample of the data to reduce overfitting and provide more robust performance estimates.
